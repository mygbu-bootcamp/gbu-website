...
// import { Eye, Share2, Download, MoreVertical } from 'lucide-react';
...
//           className={`px-4 py-2 rounded-full font-medium transition ${activeTab === 'publications' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
//         >
//           Research Publications
//         </button>
//         <button
//           onClick={() => setActiveTab('patents')}
//           className={`px-4 py-2 rounded-full font-medium transition ${activeTab === 'patents' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}
//         >
//           Patents
//         </button>
//       </div>
...
//         <select className="border border-gray-300 rounded px-4 py-2" value={selectedSchool} onChange={(e) => setSelectedSchool(e.target.value)}>
...
//         <select className="border border-gray-300 rounded px-4 py-2" value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)}>
...
//            {/* Show Type filter only in Publications tab */}
//   {activeTab === 'publications' && (
//     <select className="border border-gray-300 rounded px-4 py-2" value={selectedType} onChange={(e) => setSelectedType(e.target.value)}>
...
//           <div key={pub.id} className="bg-white shadow rounded p-6 relative">
//             {/* Actions Icon */}
//             <div className="absolute bottom-4 right-4 cursor-pointer">
//               <div className="flex gap-4 mt-4">
//                 <button className="flex items-center gap-1 text-blue-600 hover:underline"><Eye size={18} />View</button>
//                 <button className="flex items-center gap-1 text-green-600 hover:underline"><Share2 size={18} />Share</button>
//                 <button className="flex items-center gap-1 text-red-600 hover:underline"><Download size={18} />Download</button>
//               </div>
//             </div>
...
//             <p><strong>School:</strong> {pub.school}</p>
...
//           <div key={patent.id} className="bg-white shadow rounded p-6 relative">
//             {/* Actions Icon */}
...
const Publications = () => {
  ...
  const filteredPublications = publications.filter(pub => {
    ...
    const matchesYear = !selectedYear || pub.year === selectedYear;
    const matchesType = !selectedType || pub.type === selectedType;
    const matchesSearch = !searchTerm || pub.title.toLowerCase().includes(searchTerm.toLowerCase()) || pub.authors.toLowerCase().includes(searchTerm.toLowerCase()) || pub.journal.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const currentItems = activeTab === 'publications'
    ? filteredPublications.slice(indexOfFirstItem, indexOfLastItem)
    : patents.slice(indexOfFirstItem, indexOfLastItem);

  const totalPages = activeTab === 'publications'
    ? Math.ceil(filteredPublications.length / itemsPerPage)
    : Math.ceil(patents.length / itemsPerPage);
  ...
  return (
    <div className="container mx-auto px-4 py-20">
      ...
      {/* Tabs */}
      <div className="flex justify-center mb-8 space-x-4">
        <button onClick={() => { setActiveTab('publications'); setCurrentPage(1); }} className={`px-4 py-2 rounded-full font-medium transition ${activeTab === 'publications' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}>Research Publications</button>
        <button onClick={() => { setActiveTab('patents'); setCurrentPage(1); }} className={`px-4 py-2 rounded-full font-medium transition ${activeTab === 'patents' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}>Patents</button>
      </div>
      ...
      {activeTab === 'publications' && (
        <div className="flex flex-wrap gap-4 mb-8">
          ...
          <input type="text" className="border border-gray-300 rounded px-4 py-2" placeholder="Search by year" value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} />
          ...
          <input type="text" className="border border-gray-300 rounded px-4 py-2 flex-1" placeholder="Search" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      )}
      ...
      {activeTab === 'publications' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {currentItems.length > 0 && currentItems.map(pub => (
            <div key={pub.id} className="bg-white shadow rounded p-6 relative">
              <div className="absolute bottom-4 right-4 cursor-pointer">
                <div className="flex gap-4 mt-4">
                  <button className="flex items-center gap-1 text-blue-600 hover:underline"><Eye size={18} />View</button>
          ...
          {currentItems.length === 0 && (
            <div className="col-span-full text-center text-gray-500">No publications found.</div>
          )}
        </div>
      )}
      ...
      {activeTab === 'patents' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {currentItems.length > 0 && currentItems.map(patent => (
            <div key={patent.id} className="bg-white shadow rounded p-6 relative">
              <div className="absolute top-4 right-4 cursor-pointer">
                <MoreVertical className="w-5 h-5 text-gray-500 hover:text-black" />
              </div>
          ...
          {currentItems.length === 0 && (
            <div className="col-span-full text-center text-gray-500">No patents found.</div>
          )}
        </div>
      )}
      ...
      {totalPages > 1 && (
        <div className="flex justify-center mt-8 gap-2">
          <button onClick={() => setCurrentPage(prev => Math.max(prev - 1, 1))} disabled={currentPage === 1} className="px-4 py-2 rounded bg-gray-200 text-gray-700">Previous</button>
          {Array.from({ length: totalPages }, (_, index) => (
            <button key={index} onClick={() => setCurrentPage(index + 1)} className={`px-4 py-2 rounded ${currentPage === index + 1 ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}>{index + 1}</button>
          ))}
          <button onClick={() => setCurrentPage(prev => Math.min(prev + 1, totalPages))} disabled={currentPage === totalPages} className="px-4 py-2 rounded bg-gray-200 text-gray-700">Next</button>
        </div>
      )}
    </div>
  );
