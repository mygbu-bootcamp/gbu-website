...
const CoursesOffered = () => {
  ...
  <Badge key={`${course.name}-${spec}-${index}`} variant="outline" className="text-xs">
    {spec}
  </Badge>
))}
  ...
  <li key={`${course.name}-${highlight}-${index}`} className="flex items-center space-x-2 text-sm text-gray-600">
  ...
  <CourseCard key={`${level}-${course.name}-${index}`} course={course} />
))}
